...
- **[制約・実機で確認済み]** claude.ai(ブラウザ/デスクトップのチャット版)のカスタムコネクタは、2026年時点ではOAuthのみ対応で、固定トークンをリクエストヘッダーとして設定する機能はアカウントによってはまだ展開されていない(ベータ機能)。本サーバーはOAuthを実装していないため、その場合claude.aiのチャットからは接続できず、**Claude Codeでの利用に限定**される。claude.aiのヘッダー入力機能が展開されるか、本サーバー側にOAuthを実装するまではこの制約が続く(§11参照)。
...
- claude.ai(チャット版)からの利用を可能にするか否か(現状は未対応と判断): claude.aiのヘッダー認証ベータ機能の展開を待つか、本サーバーにOAuth 2.0(認可サーバー、トークン発行等)を実装するかの二択。個人利用規模での実装コストと見合わないと判断し、当面はClaude Codeでの利用に限定する(§3参照)
